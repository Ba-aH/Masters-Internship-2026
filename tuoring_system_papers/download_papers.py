#!/usr/bin/env python3
"""
Paper Downloader v4 - Maximum Coverage Edition
Search order: DOI first → title fallback
Sources: OpenAlex, Semantic Scholar, Unpaywall, ArXiv, CORE (with key),
         Europe PMC, BASE (new), Google Scholar, Publisher Direct, Sci-Hub
...
CORE_API_KEY  = "5qo9HW0V2xUMXSNtY7fpKQJGvuzj1kCR"  # Your CORE key
TIMEOUT       = 30
DELAY         = 2.8       # seconds between papers
MIN_PDF_SIZE  = 5_000     # bytes — reject tiny HTML pages
RETRIES       = 2         # per download attempt
# ─────────────────────────────────────────────────────────────────────────────
...
def fetch_pdf(url: str) -> bytes | None:
    """Download URL with retries; return bytes if valid PDF."""
    for attempt in range(RETRIES + 1):
        try:
            ...
            print(f"     ✗ Fetch failed (attempt {attempt+1}): {str(e)[:80]}")
            time.sleep(DELAY * 2)
    return None
...
# ══════════════════════════════════════════════════════════════════════════════
#  SOURCE 5 — CORE.AC.UK (v3 with key)
# ══════════════════════════════════════════════════════════════════════════════
...
def core_doi(doi_clean: str) -> str | None:
    try:
        ...
        r = requests.get(
            f'https://api.core.ac.uk/v3/works/{quote(doi_clean)}',
            headers=headers, timeout=TIMEOUT)
        if r.status_code == 200:
            data = r.json()
...
def base_doi(doi_clean: str) -> str | None:
    try:
        ...
        r = requests.get("https://api.base-search.net/cgi-bin/BaseSearch/rsearch",
        ...
        if not docs:
            return None
        doc = docs[0]
        if doc.get('dcformat') == 'application/pdf' and doc.get('dclink'):
            return doc['dclink']
        # fallback: look for PDF in dcsource or dcterms
        for link in doc.get('dclink', []):
            if isinstance(link, str) and 'pdf' in link.lower():
                return link
    except Exception as e:
        print(f"     BASE DOI error: {str(e)[:80]}")
    return None
...
def base_title(title: str) -> str | None:
    try:
        ...
        print(f"     BASE title error: {str(e)[:80]}")
    return None


# ══════════════════════════════════════════════════════════════════════════════
#  SOURCE 7 — GOOGLE SCHOLAR
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  SOURCES TABLE  — ordered by reliability / coverage
# ══════════════════════════════════════════════════════════════════════════════
...
SOURCES = [
    ...
    ('BASE',             base_doi,        base_title),           # ← NEW: BASE added here
